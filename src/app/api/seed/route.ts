...
export async function POST(req: NextRequest) {
  try {
    ...
    // Seed videos — createdAt is strictly in the past (sorted desc by createdAt).
    const now = Date.now();
    const videoIds: string[] = [];
    for (let i = 0; i < DEMO_VIDEOS.length; i++) {
      const v = DEMO_VIDEOS[i];
      const id = db.generateId();
      // Stagger createdAt over the last 30 days so videos sort nicely.
      const createdAt = now - i * 60_000 - Math.floor(Math.random() * 60_000);
    ...
    // Seed demo gift codes — include the `code` field (unified with /api/admin/gift-codes).
