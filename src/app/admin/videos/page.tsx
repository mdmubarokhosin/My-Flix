...
import type { Video, DownloadLink } from '@/lib/types';
...
import {
  Plus, Pencil, Trash2, Search, Film, Wand2, Star, Loader2,
  Download, Link2, GripVertical,
} from 'lucide-react';
import { toast } from 'sonner';

// Common lucide icon names for the download-button picker.
...
export default function VideosPage() {
  ...
  const openEdit = (item: Video) => {
    ...
    setForm({
      ...
      downloadLinks: Array.isArray(item.downloadLinks) ? item.downloadLinks : [],
    });
    setSearchQuery('');
    setSearchResults([]);
    setDialogOpen(true);
  ...
  const handleSave = async () => {
    // Only the name is required. URL can be added later (e.g. when admin
    // imports metadata first, then sources a streaming link afterwards).
    if (!form.name.trim()) {
    ...
    setSaving(true);
    try {
      // Filter out empty download links.
      const cleanLinks = form.downloadLinks
      ...
      const payload: Record<string, unknown> = {
        name: form.name.trim(),
        url: form.url.trim(), // may be empty — server allows it
    ...
    try {
      if (importSource === 'tmdb') {
        ...
        if (isTmdbId) {
          ...
          else toast.error((data as { error?: string }).error || 'Movie not found');
        } else {
  ...
  // ---- Apply TMDB or IMDB search result to the form (auto-fill everything) ----
  const applyImportResult = async (item: Record<string, unknown>) => {
    if (importSource === 'tmdb') {
      ...
      const runtime = item.runtime as number | undefined;
      const imdbId = item.imdb_id as string | undefined;
      ...
      // Format duration: TMDB gives minutes → "2h 15m"
      let durationStr = '';
      if (runtime) {
      ...
      setForm(prev => ({
        ...
        year: releaseDate ? releaseDate.split('-')[0] : prev.year,
        ...
        cast: cast.join(', '),
        director,
        contentType: prev.contentType || 'movie',
      }));
      toast.success('TMDB data applied — all fields filled!');
    } else {
      // IMDB (OMDb)
      const title = item.Title as string;
      ...
      toast.success('IMDB data applied — all fields filled!');
    }
    setSearchResults([]);
    setSearchQuery('');
  };
    ...
    <div className="@container/main flex flex-col gap-4 md:gap-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-muted-foreground">Manage your movie collection. Import from TMDB or IMDB for auto-fill.</p>
        </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              {editingId ? 'Update video details below.' : 'Add a new movie. Use Auto-Fill Import to fetch data from TMDB or IMDB.'}
            </DialogDescription>
          </DialogHeader>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-2">
            {/* TMDB / IMDB Auto-Fill Import Section */}
            <div className="md:col-span-2 p-3 rounded-lg bg-muted/30 border border-border/50">
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-[10px] text-muted-foreground ml-auto">
                  Fills name, thumbnail, image, year, language, duration, rating, genres, cast, director, synopsis, TMDB/IMDB IDs.
                </span>
              </div>
                    ...
                    {searchResults.slice(0, 8).map((m, idx) => (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-medium line-clamp-1">{(m.title as string) || (m.Title as string)}</p>
                          <p className="text-[10px] text-muted-foreground">
                            {importSource === 'tmdb'
                              ? `${(m.release_date as string)?.split('-')[0] || '?'} · ⭐ ${(m.vote_average as number)?.toFixed(1) || '?'}${m.imdbID ? ` · ${m.imdbID}` : ''}`
                              : `${(m.Year as string) || '?'} · ⭐ ${(m as { imdbRating?: string }).imdbRating || '?'} · ${(m.imdbID as string) || ''}`
                            }
                          </p>
                        </div>
            ...
            <div className="md:col-span-2 space-y-2">
              <Label>Video URL (streaming / embed URL — optional)</Label>
              <Input value={form.url} onChange={e => setForm({ ...form, url: e.target.value })} placeholder="https://... (leave empty to add later)" />
              <p className="text-[11px] text-muted-foreground">
                You can save the video with metadata first, then add the streaming URL later.
              </p>
            </div>
            ...
            <div className="space-y-2">
              ...
              <Input value={form.rating} onChange={e => setForm({ ...form, rating: e.target.value })} placeholder="8.5" />
            </div>
            <div className="space-y-2">
              <Label>TMDB ID</Label>
            ...
            <div className="space-y-2">
              ...
              <Input value={form.director} onChange={e => setForm({ ...form, director: e.target.value })} placeholder="Director name" />
            </div>
            <div className="space-y-2">
              <Label>Genres (comma separated)</Label>
              <Input value={form.genres} onChange={e => setForm({ ...form, genres: e.target.value })} placeholder="Action, Thriller" />
            </div>
            <div className="md:col-span-2 space-y-2">
              <Label>Cast (comma separated)</Label>
              <Input value={form.cast} onChange={e => setForm({ ...form, cast: e.target.value })} placeholder="Actor 1, Actor 2, ..." />
            </div>
            <div className="md:col-span-2 space-y-2">
            ...
            <div className="md:col-span-2 space-y-3">
              <div className="flex items-center justify-between">
                <div>
                  <Label className="flex items-center gap-1.5"><Download className="w-3.5 h-3.5" /> Download Links</Label>
                  <p className="text-[11px] text-muted-foreground mt-0.5">
                    Custom download buttons shown to users. Each button has a name, icon, and URL.
                  </p>
                </div>
                    ...
                    <div key={idx} className="grid grid-cols-12 gap-2 p-2 rounded-lg border bg-muted/20">
                      ...
                      <div className="col-span-6 sm:col-span-2">
                        ...
                        <Select
                          ...
                          onValueChange={v => updateDownloadLink(idx, 'icon', v)}
                        >
                      ...
                      <div className="col-span-6 sm:col-span-2">
                        <Label className="text-[10px] text-muted-foreground">Quality (optional)</Label>
                        <Input
                          value={link.quality || ''}
                          onChange={e => updateDownloadLink(idx, 'quality', e.target.value)}
                          placeholder="720p"
                          className="text-sm h-8"
                        />
                      </div>
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This will permanently delete this video and remove it from all users' purchases and favorites.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
