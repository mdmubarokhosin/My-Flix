...
export interface Episode {
  ...
  thumbnail?: string;
  duration?: string;
  /** Optional per-episode download links (overrides the series-level list). */
...
/**
 * A custom download button attached to a video (or episode).
 * Admin specifies the button label, an icon name (lucide), and the URL.
 */
export interface DownloadLink {
  /** Button label, e.g. "Download 720p", "Direct MP4", "GDrive". */
  label: string;
  /**
   * Icon name from lucide-react (e.g. "Download", "FileVideo", "HardDriveDownload",
   * "CloudDownload", "Link"). Falls back to a generic Download icon if unknown.
   * Also accepts Bootstrap Icons classes (e.g. "bi bi-download") — those are
   * rendered via the CDN stylesheet.
   */
  icon?: string;
  /** The actual download URL (http/https). */
  url: string;
  /** Optional quality label, e.g. "720p", "1080p", "4K". */
  quality?: string;
  /** Optional file size hint, e.g. "850 MB". */
  size?: string;
}
...
export interface Video {
  ...
  /** Custom download buttons (movies + series). */
  downloadLinks?: DownloadLink[];
  /** Optional backdrops/gallery images from TMDB/IMDB. */
  backdrops?: string[];
  /** Optional cast list (string[]) — pulled from TMDB credits. */
  cast?: string[];
  /** Optional director/creator. */
  director?: string;
  /** Optional genres (string[]) — pulled from TMDB. */
  genres?: string[];
  /** Optional TMDB vote average (0-10). */
  rating?: number;
}
...
export interface UserTransaction {
  type: 'earn' | 'spend' | 'redeem' | 'gift' | 'admin' | 'checkin' | 'purchase' | 'ad';
  title: string;
  amount: number;
  time: number;
}
...
export interface GiftCode {
  id: string;
  code?: string;         // The XXXX-XXXX-XXXX string the user types. Same as id for legacy codes.
  amount: number;
  package: string;
  status: 'active' | 'used';
  redeemedBy?: string;
...
export interface Settings {
  categories: Category[];
  adminPassword?: string | null;        // Legacy plaintext (migrated to hash on first login).
