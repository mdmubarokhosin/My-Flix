 ...
 * The list is trimmed to the most recent 50 entries to keep the user
 * document small. Does NOT update the user's balance — the caller should
 * handle that separately (ideally via an atomic multi-location update).
 */
export async function addTransaction(
  ...
  const existing = await database.get<UserTransaction[] | Record<string, UserTransaction> | null>(
  ...
  if (Array.isArray(existing)) {
    ...
    transactions = [transaction, ...Object.values(existing)];
  } else {
