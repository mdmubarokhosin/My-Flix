...
import { normalizeVideo } from '@/lib/normalize-video';
...
export function VideoPlayer() {
  ...
  const video = useMemo<Video | null>(() => {
    // Priority 1: episode video passed via navigationData (series episodes).
    const episodeVideo = navigationData.episodeVideo as Video | undefined;
    if (episodeVideo && String(episodeVideo.id) === String(videoId)) {
      return episodeVideo;
    }
    // Priority 2: currentVideo from store.
    if (currentVideo && String(currentVideo.id) === String(videoId)) return currentVideo;
    // Priority 3: look up in the videos array.
    return videos.find((v) => String(v.id) === String(videoId)) ?? null;
  }, [currentVideo, videoId, videos, navigationData]);

  // Fallback direct fetch if video not in state yet.
  // Skip the fetch entirely for composite episode IDs (series episodes)
  // — they are passed via navigationData.episodeVideo and have no real
  // `videos/<id>` document in Firebase.
  useEffect(() => {
    if (!video && videoId && !videoId.includes('::')) {
      let active = true;
      const fetchDirect = async () => {
        try {
          // 1. Try Realtime Database (uses normalizeVideo so ALL fields
          //    including downloadLinks, director, genres, cast, rating are
          //    preserved — not just the original subset).
          ...
          if (snapshot.exists() && active) {
            ...
            const fetched = normalizeVideo(val, videoId);
            if (fetched) {
          ...
          // 2. Try API route (returns the full record from Firebase)
          const res = await fetch(`/api/videos/${videoId}`);
          if (res.ok && active) {
            const val = (await res.json()) as Record<string, unknown>;
            if (val && val.id) {
              const fetched = normalizeVideo(val, String(val.id));
              if (fetched) {
                setCurrentVideo(fetched);
                return;
              }
  ...
  if (!video || !resolvedVideo) {
    ...
    // If video exists but URL is empty/broken — still show download links
    // (if any) so the user can grab the file even without streaming.
    if (video && !resolvedVideo) {
      const safeLinks = (video.downloadLinks || []).filter(
      ...
      return (
        <div className="w-full px-4 pt-4 pb-24">
          ...
          {/* Video metadata + download links (shown even without a streaming URL) */}
          <div className="mt-6 space-y-4">
            <div>
              <h1 className="text-xl font-bold mb-2">{video.name}</h1>
              {(video.year || video.language || video.quality) && (
              ...
              {video.info && video.info.length > 0 && (
                <p className="text-sm text-muted-foreground mt-2">{video.info.join(' ')}</p>
              )}
            </div>
  ...
  return (
    <div className="w-full pb-24">
      ...
      <div className="relative rounded-b-2xl overflow-hidden shadow-md">
        {needsPurchase ? (
            ...
            <div className="relative z-10 flex flex-col items-center max-w-sm px-4">
              ...
              {!hasEnoughCoins && (
                <p className="text-[11px] text-red-400 mt-2.5 font-medium">
                  {t('player.notEnoughCoinsFull', lang).replace('{more}', String((video?.amount ?? 0) - (user?.balance ?? 0)))}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="relative">
            {resolvedVideo.type === 'mp4' || resolvedVideo.type === 'hls' ? (
              <video
                key={`${resolvedVideo.src}-${videoRetryKey}`}
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        {/* Download Links — only render the section if there's at least one
            valid http(s) download URL. DownloadButtons itself also filters
            unsafe URLs, but we add the same check here to avoid rendering
            an empty section + separator. */}
        {(() => {
          const safeLinks = (video.downloadLinks || []).filter(
            (l) => l && typeof l.url === 'string' && /^https?:\/\//i.test(l.url),
          );
