...
export function SeriesDetailPage() {
  ...
  const handlePlayEpisode = (episode: Episode) => {
    ...
    // IMPORTANT: pass `episodeVideo` as navigationData so VideoPlayer uses
    // it directly instead of fetching `videos/${series.id}` from Firebase
    // (which would lose episode.url).
    const episodeVideo: Video = {
      ...series,
      id: `${series.id}::s${expandedSeason}e${episode.episodeNumber}`,
    ...
    setPage('player', { videoId: episodeVideo.id, episodeVideo });
  };
    ...
    <div className="pb-24">
      ...
      <div className="relative">
        ...
        <div className="px-4 -mt-16 relative z-10">
          ...
          {/* Series-level download links (full-season packs, etc.) */}
          {(() => {
            const safeLinks = (series.downloadLinks || []).filter(
            ...
            return (
              <div className="mt-4">
                <DownloadButtons
                  links={safeLinks}
                  variant="row"
                  title={t('player.downloadLinks', lang)}
                />
              </div>
            );
          })()}
        ...
        <div className="space-y-2">
          ...
          {activeSeason?.episodes.map((episode) => (
            <div
              ...
              className="flex items-start gap-3 p-3 rounded-xl bg-card border border-border/50 hover:border-primary/30 transition-all group cursor-pointer"
            ...
            >
              ...
              <div className="flex-1 min-w-0 py-0.5">
                ...
                {(() => {
                  const epLinks = (episode.downloadLinks || []).filter(
                    (l) => l && typeof l.url === 'string' && /^https?:\/\//i.test(l.url),
                  );
                  ...
                  return (
                    ...
                    >
                      <DownloadButtons links={epLinks} variant="row" />
                    </div>
                  );
                })()}
              </div>
