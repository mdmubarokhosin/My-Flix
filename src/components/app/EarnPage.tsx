...
export function EarnPage() {
  const { user, userId, setPage, lang } = useAppStore();
  // Single shared Firebase listener for user data (idempotent across components).
  useUserData();
  const [isCheckingIn, setIsCheckingIn] = useState(false);
  const [checkInResult, setCheckInResult] = useState<{ coins: number; streak: number } | null>(null);
  const [isWatchingAd, setIsWatchingAd] = useState(false);
  const [adCountdown, setAdCountdown] = useState(0);
  const [adWatchesToday, setAdWatchesToday] = useState(user?.adWatchedToday ?? 0);
  const [transactions, setLocalTransactions] = useState<UserTransaction[]>(user?.transactions ?? []);
  ...
  // Sync local state when the store's user updates (single source of truth).
  useEffect(() => {
    if (user) {
      setLocalTransactions(user.transactions ?? []);
      setAdWatchesToday(user.adWatchedToday ?? 0);
      setTxLoading(false);
    } else {
      setTxLoading(false);
    }
  ...
  const handlePurchase = useCallback(async (pkg: CoinPackage) => {
    ...
    try {
      const res = await fetch('/api/payment', {
        ...
        body: JSON.stringify({ userId, packageId: pkg.id, userName: user?.firstName || undefined }),
      });
  ...
  const handleCheckIn = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/user/checkin?tzOffset=${tzOffset}`, {
        method: 'POST',
    ...
    adTimerRef.current = setInterval(() => {
      setAdCountdown((prev) => {
        if (prev <= 1) {
          ...
          // Server-side determines the reward amount (settings.coinsPerAd).
          // Pass tzOffset so the daily-reset is timezone-aware.
          const tzOffset = -new Date().getTimezoneOffset();
          fetch(`/api/user/transactions?tzOffset=${tzOffset}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ userId, type: 'ad', title: 'Watched ad' }),
          })
            .then((r) => r.json())
            .then((data) => {
              if (data.success) {
                setAdWatchesToday(data.adsToday ?? adWatchesToday + 1);
                toast.success(`+${data.coins ?? adCoins} coins from ad!`);
              } else if (data.error) {
