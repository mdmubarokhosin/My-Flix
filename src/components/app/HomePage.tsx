...
export function HomePage() {
  ...
  useEffect(() => {
    ...
    const unsubVideos = onValue(videosRef, (snapshot) => {
      ...
      if (data) {
        // Use normalizeVideo so ALL fields (including the new downloadLinks,
        // director, genres, cast, rating, backdrops) are preserved.
        const vids: Video[] = Object.entries(data)
          .map(([id, v]) => normalizeVideo(v, id))
  ...
  // Single shared user listener (defined in useUserData). Sync favorites +
  // purchases from the store rather than subscribing a second time here.
  useUserData();
  useEffect(() => {
    if (user) {
  ...
  }, [user, setFavorites, setPurchases]);
